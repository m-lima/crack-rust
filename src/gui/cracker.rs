...
impl Cracker {
    fn crack(
        ...
        let input = input
            ...
            .filter_map(|s| hash::sha256::Hash::from_str(&s.to_string()).ok())
            .collect();

        let files = files
            .into_iter()
            .filter_map(|v| qmetaobject::QString::from_qvariant(v.clone()))
            .filter_map(|s| std::path::PathBuf::from_str(&s.to_string()).ok())
            .collect();
        ...
        match options::Decrypt::new(input, files, maybe_salt, length, prefix, None, maybe_device) {
            Ok(options) => {
                self.run_algorithm(&options);
            }
            Err(err) => self.error(err.to_string()),
        }
    }

    fn run_algorithm<H: hash::Hash>(&self, options: &options::Decrypt<H>) {
        if let Err(err) = decrypt::execute(options, self) {
