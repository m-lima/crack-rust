...
use qt_widgets::qt_core::{
    qs, QBox, QSignalBlocker, QStringList, Signal, SlotNoArgs, SlotOfBool, SlotOfInt, SlotOfQString,
};
use qt_widgets::qt_gui::{QFont, QIntValidator};
use qt_widgets::{
    q_message_box, QApplication, QButtonGroup, QComboBox, QFileDialog, QFormLayout, QGridLayout,
    QGroupBox, QHBoxLayout, QInputDialog, QLabel, QLineEdit, QListWidget, QListWidgetItem,
    QMessageBox, QPlainTextEdit, QPushButton, QRadioButton, QSpinBox, QTabWidget, QVBoxLayout,
    QWidget,
};
...
impl channel::Channel for Printer {
...
//         QMessageBox::from_icon2_q_string_q_flags_standard_button(
//             q_message_box::Icon::Warning,
//             &qs("An error occurred"),
//             &qs(message),
...
unsafe fn crack_tab(parent: &QBox<QWidget>, font: &CppBox<QFont>) -> QBox<QWidget> {
    ...
    let toggled = SlotOfBool::new(&root, move |value| {
        println!("{:?}", value);
    });
    is_sha256.connect(&toggled);
